...
extension String {
	...
	func between(_ start: String, _ end: String) -> StringProperty? {
		var range = NSRange()
		var flag: Bool = false
		let string = self as NSString

		for i in 0 ..< string.length {
			let subStr = string.substring(with: NSRange(location: i, length: 1))
			if start == subStr {
				range.location = i
				flag = true
				continue
				...
				let value = string.substring(with: strRange)
