...
extension NSAttributedString {
	...
	func getPlainString() -> String {
		...
		var base: Int = 0
		self.enumerateAttribute(NSAttachmentAttributeName, in: NSMakeRange(0, self.length), options: []) { (value, range, stop) -> Void in
			if let value = value as? EmojiTextAttachment {
				if let emojiTag = value.emojiTag {
					plainStr.replaceCharacters(in: NSMakeRange(range.location + base, range.length), with: emojiTag)
